...
get_nearby_snps <- function(dat, lead_snp_annot, n_snps = 1000, max_distance = 10^6){
    nearby_snps <- dat %>%
        ...
        dplyr::filter(abs(base_pair_location - lead_snp_annot$position) < max_distance) %>%
        dplyr::arrange(abs(base_pair_location - lead_snp_annot$position)) %>%
...
#' 
#' @param chr a character vector with one entry per SNP - the chromosome
#' @param pos a numeric vector of snp positions on the chromosome chr
#' @param ref_allele a character vector with one entry per SNP - the reference allele
#' @param alt_allele a character vector with one entry per SNP - the alternate allele
#' @return a tibble with columns 'chr', 'pos', 'ref_allele', 'alt_allele', 'rsid'
get_rsid_from_snp_info <- function(chr, pos, ref_allele, alt_allele){
    # get rsid from chromosome and position
    # load the GRCh37 db
    txdb <- TxDb.Hsapiens.UCSC.hg19.knownGene::TxDb.Hsapiens.UCSC.hg19.knownGene
    snpg <- GenomicRanges::GRanges(seqnames = chr[1], ranges = IRanges::IRanges(start = pos[1], end = pos[1]), strand = "*",
                ref = ref_allele[1], alt = alt_allele[1], txdb = txdb)
    snpdb <- AnnotationDbi::loadDb("dbSNP")
    snpg <- annotateWith(rsid(snpdb), snpg)

    tib <- ref_seq %>%
        as.character() %>%
        toupper() %>%
        tibble::as_tibble() %>%
        dplyr::rename(ref_allele_per_BSgenome = value) %>%
...
for (outcome in outcomes){
    ...
    for (row_num in seq_len(nrow(sa))){ #loop over lead SNPs
        ...
        one_joined <- one_snp_tib %>%
            dplyr::inner_join(barton_tib, by = c("chr" = "chromosome", "position" = "base_pair_location"))
        if (nrow(one_joined) == 1){
            proxies[[rs]] <- one_snp_tib
        } else {
            # if lead SNP is not in barton data, then we need a proxy for it 
            snp_tib <- get_nearby_snps(dat = barton_tib, lead_snp_annot = one_snp_tib, n_snps = 99)
            ...
            ld_tib <- get_ld_matrix(snps = c(rs, st2$refsnp_id), 
                                    ld_token = ld_token) %>%
                tibble::as_tibble()    
            ld_tib_small <- organize_ldlink_output(ldlink_output = ld_tib) %>%
                dplyr::filter(column_name == rs)
            # get rs id for SNPs in the barton data
            
            # see if best proxies are in fmd data 
             foo <- get_proxies_tib(ldlink_output = ld_tib_small, ld_threshold = 0.9)
             proxies[[rs]]
        }
