...
for (file in files){
    ...
    for (rs in fmd_lead_snps){
        ...
        ld_tib <- get_ld_matrix(snps = rsids, 
         ...
         pf2 <- proximal_fmd %>% 
            dplyr::relocate(p_value, .after=SNP) %>% 
            dplyr::filter(p_value < 10^-5) %>%
            dplyr::mutate(in_dat_rs = SNP %in% dat$rsids, 
                          in_dat_chr_pos = chr_pos %in% proximal_dat$chr_pos) %>%
            dplyr::relocate(in_dat_rs, in_dat_chr_pos, .after = p_value) %>%
            dplyr::relocate(chromosome, dist_to_lead, .after = in_dat_chr_pos)     
    }
}
```
