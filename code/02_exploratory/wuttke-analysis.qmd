...
for (file in files){
    ...
    for (rs in fmd_lead_snps){
        ...
        rsids <- c(rs, proximal_fmd$SNP, proximal_dat$rsids) %>% unique() %>% (function(x) x[!is.na(x)])    
        ld_tib <- get_ld_matrix(snps = rsids, ld_token = ld_token) %>%
            tibble::as_tibble()        
        ld_small <- organize_ldlink_output(ld_tib) %>%
            dplyr::filter(column_name == rs) %>%
            dplyr::filter(!(column_name == row_name)) %>%
            dplyr::arrange(dplyr::desc(value)) %>%
            dplyr::mutate(row_in_dat = row_name %in% proximal_dat$rsids) %>%
            dplyr::filter(row_in_dat) %>%
            dplyr::slice(1)
        tibs[[rs]] <- ld_small        
    }
}
