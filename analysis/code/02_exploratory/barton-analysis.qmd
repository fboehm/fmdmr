...
for (chr in unique(fmd_leads$chromosome)){
    ...
    if (nrow(fmd_leads_chr) <= 1){
        ...
        ld_tib <- get_ld_tib(snps = fmd_leads_chr$SNP, 
            genome_build = "grch37", 
            token = ld_token)
        out <- choose_one_from_corr_snps(snp_info = fmd_leads_chr, ld_tibble = ld_tib, ld_threshold = 0.8)
        fl2[[chr]] <- out
    }
}
...
corr_snps_for_leads <- get_correlated_snps(snps = indep_not_in_dat$SNP, 
                                            genome_build = "grch37", 
                                            ld_token = ld_token) 
