---
title: "CKD as a possible consequence of FMD"
format: gfm
---
...
```{r}
library(magrittr) # needed for TwoSampleMR functions to work properly!!! ie, they set up the package without exporting magrittr pipe, I think
```
...
# check if the mrcieu database has the snps of interest iwth ieugwasr::ld_reflookup
fmd2pre$SNP |>
    ieugwasr::ld_reflookup()
# all 7 snps are in the database!

fmd2 <- fmd2pre |>
    TwoSampleMR::clump_data()
files <- list.files(here::here("analysis", "data", "ukb_for_munge_sumstats"), pattern = "female", full.names = TRUE)
file <- files[3] # check this!
...
mydir <- here::here("analysis", "data", "analysis_intermediates") 
mydir |> fs::dir_create()
saveRDS(dat_harm, file.path(mydir, "fmd_ckd_dat_harm.rds"))
