...
results <- results_file |>
    vroom::vroom(col_names = cnames, skip_empty_rows = FALSE, trim_ws = FALSE)
```


```{r}    
r2 <- results |>
    dplyr::filter(!is.na(phenotype1_file)) |>
    dplyr::arrange(p) |>
    ...
    dplyr::mutate(phenotype1_file_without_path_v2 = stringr::str_remove(string = phenotype1_file_without_path, 
                                                                        pattern = ".v2")) |>
    dplyr::mutate(period_count_v2 = stringr::str_count(phenotype1_file_without_path_v2, "\\.")) |>
    dplyr::mutate(dilution_factor_present = stringr::str_detect(string = phenotype1_file_without_path_v2, 
                                                                pattern = "dilution_factor")) |>
    dplyr::mutate(phenotype1_file_without_path_v3 = stringr::str_remove(string = phenotype1_file_without_path_v2, 
                                                                        pattern = ".dilution_factor")) |>
    tidyr::separate_wider_delim(col = phenotype1_file_without_path_v3, 
...
manifest <- vroom::vroom(manifest_file) |>
    # define new variable for dilution_factor_present
    dplyr::mutate(dilution_factor_present = stringr::str_detect(string = ldsc_sumstat_file, pattern = ".dilution_factor")) |>
    dplyr::relocate(dilution_factor_present, dilute, ldsc_sumstat_file) |>
    dplyr::arrange(dilute)
manifest$dilution_factor_present |> 
    table()
# manually inspect file names to see if dilute 
...
out <- r2 |>
    dplyr::inner_join(manifest, by = dplyr::join_by(phenotype1_id == phenotype, 
                                                    sex, 
                                                    dilution_factor_present,
                                                    v2_present == is_v2)) |> 
